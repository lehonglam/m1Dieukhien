...
{
    [clsTaskMetaInfo("BPB Set Audio Volume Level", "ClsBPB_SetAudioVolumeLevel", true)]
    public class ClsBPB_SetAudioVolumeLevel : ITask
    {
        ...
        public ClsBPB_SetAudioVolumeLevel()
        {
            m_bStopWhenFail = true;
            AllowExcution = true;
            m_strDescription = "BPB Set Audio Volume Level";
            VolumeLevel = 0;
            RetryTimes = 3;
            Result_unit = "";
        }
        ...
        public string Name
        {
            get { return "ClsBPB_SetAudioVolumeLevel"; }
        }
        [Browsable(false)]
        ...
        public TaskResult Excution(CFTWinAppCore.DeviceManager.IDeviceManager DevManager)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < RetryTimes; i++)
                {
                    ...
                    if (!bOk)
                    {
                        m_strDisplayValue = "SetAudioVolume Fail";
                        continue;
                    }
                    Thread.Sleep(1000);
                    break;
                ...
                if (!bOk)
                {
                    clsLogManager.LogWarning("SetAudioVolume {0} Fail!", VolumeLevel.ToString());
                    if (clsMsgHelper.ShowYesNo("Question", "Set up fail, bạn có thực hiện lại không?") == DialogResult.No)
                    {
                        return TaskResult.FAIL;
                    }
        ...
        public ITask Clone()
        {
            ClsBPB_SetAudioVolumeLevel task = new ClsBPB_SetAudioVolumeLevel();
