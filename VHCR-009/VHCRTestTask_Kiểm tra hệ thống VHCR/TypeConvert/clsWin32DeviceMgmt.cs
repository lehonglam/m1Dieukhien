...
{
    public struct WinDeviceInfo
    {
        public string name;
        public string decsription;
    }
    public class clsWin32DeviceMgmt
    {
        ...
        public static List<WinDeviceInfo> GetAllCOMPorts()
        {
            ...
            try 
            {
                List<WinDeviceInfo> devices = new List<WinDeviceInfo>();
                ...
                while (true)
                {
                    ...
                    clsLogManager.LogReport("deviceInfoData: Inforset {0} - Index: {1} - InfoData: {2}", hDeviceInfoSet.ToString(), iMemberIndex, deviceInfoData.ToString());
                    if (!success)
                    {
                        // No more devices in the device information set
                        clsLogManager.LogError("suceess = false");
                        break;
                    }
 
                    WinDeviceInfo deviceInfo = new WinDeviceInfo();
                    ...
                    clsLogManager.LogWarning("Add {0} - {1}: DONE", deviceInfo.name, deviceInfo.decsription);
                    iMemberIndex++;
                }
                return devices;
            }
            ...
            {
                MessageBox.Show("GetAllCOMPorts() catched an exception: " + ec.Message);
                return null;
            }
            finally {
